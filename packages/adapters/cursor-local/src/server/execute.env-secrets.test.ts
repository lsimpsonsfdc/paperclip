...
const {
  ...
  runAdapterExecutionTargetProcess: vi.fn(async () => ({
    ...
    timedOut: false,
    stdout: [
      JSON.stringify({ type: "result", subtype: "success", session_id: "cursor-session-1", result: "ok" }),
    ].join("\n"),
    stderr: "",
    pid: 123,
...
describe("cursor platform signing secret containment", () => {
  ...
  it("keeps platform signing secrets out of the child env while the run still completes", async () => {
    const cwd = await mkdtemp(path.join(os.tmpdir(), "paperclip-cursor-env-"));
    ...
    const result = await execute({
      ...
      config: { command: "agent", cwd, promptTemplate: "Follow the paperclip heartbeat." },
      context: {},
      onLog: async () => {},
    } as never);

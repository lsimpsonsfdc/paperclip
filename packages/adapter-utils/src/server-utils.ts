...
// Exported so the agent-env drift test can reuse exactly the shape the log
// redactor considers sensitive, instead of keeping a second copy that drifts.
...
// ── Agent env chokepoint (SSO-22654) ────────────────────────────────────────
//
// Local adapters must inherit the Paperclip server's environment (that is how
// provider credentials, PATH, HOME and the rest reach the agent CLI), but a
// handful of host vars are platform secrets that no agent process has any
// business seeing. Before this helper existed, every call site hand-rolled
// `{ ...process.env, ...env }` — 26 independent copies — so a secret added to
// the server env silently reached every agent child process, and an `env`/
// `printenv` in a transcript exfiltrated it (SSO-21704).
//
// `buildInheritedAgentEnv()` is the single construction point for a child agent
// environment. Every `...process.env` merge in `packages/adapter-utils` and
// `packages/adapters` routes through it; `agent-env-chokepoint.test.ts` fails
// the build if a new hand-rolled copy appears.
//
// Polarity is deliberately a DENY-LIST, not an allowlist: these paths must keep
// inheriting provider credentials (CLAUDE_CODE_OAUTH_TOKEN, ANTHROPIC_API_KEY,
// GEMINI_API_KEY, …) and an allowlist would break every local adapter on day
// one. Deny-by-default — routing everything through
// `sanitizeInheritedPaperclipEnv()`'s prefix-strip-plus-allowlist — is a
// separate change with its own test matrix.
//
// SCOPE: this closes the *accidental* propagation path only. A deliberate
// reader inside the agent process shares the host PID/user namespace and can
// still read `/proc/1/environ` or the host `.env` file. uid / PID-namespace
// separation is tracked on SSO-22650.

/**
 * Host env vars that must never be inherited by an agent child process.
 * Additions here are security-relevant: keep the justification comment.
 */
export const AGENT_ENV_DENY_LIST: readonly string[] = [
  // Signs agent run JWTs. An agent holding this can mint a token for any agent
  // in any company on this control-plane instance.
  "PAPERCLIP_AGENT_JWT_SECRET",
  // better-auth session signer for human web sessions. Session forgery for any
  // operator, including the Board.
  "BETTER_AUTH_SECRET",
  // Signs tool-action approvals. Lets an agent self-approve gated actions.
  "PAPERCLIP_TOOL_ACTION_SIGNING_SECRET",
];

const AGENT_ENV_DENY_SET = new Set(AGENT_ENV_DENY_LIST.map((key) => key.toUpperCase()));

/**
 * Sensitive-shaped keys (per `SENSITIVE_ENV_KEY`) that local adapters
 * deliberately inherit into the agent process. Every entry is reviewed: it is
 * either a provider credential the agent CLI needs to authenticate, or a
 * per-run value Paperclip mints for that specific run.
 *
 * `agent-env-chokepoint.test.ts` scans adapter sources for sensitive-shaped env
 * keys and fails when one survives `buildInheritedAgentEnv()` without being
 * listed here — that drift test, not the runtime, is what stops the next secret
 * from leaking silently.
 */
export const REVIEWED_INHERITED_SENSITIVE_ENV_KEYS: readonly string[] = [
  // Provider credentials: the agent CLI authenticates with these directly.
  "ANTHROPIC_API_KEY",
  ...
  "XAI_API_KEY",
  "ZAI_API_KEY",
  // Per-run Paperclip values, minted for this run and scoped to this agent.
  // Not platform secrets: leaking one costs at most the run it belongs to.
  "PAPERCLIP_API_KEY",
  "PAPERCLIP_BRIDGE_API_KEY",
  "PAPERCLIP_BRIDGE_TOKEN",
  "PAPERCLIP_CLOUD_PROVIDER_TOKEN",
  // Adapter self-test probe markers, set by Paperclip's own connection tests.
  "PAPERCLIP_CODEX_TEST_PROCESS_KEY",
  "PAPERCLIP_CODEX_TEST_RUN_KEY",
  "PAPERCLIP_PI_TEST_KEY",
];

export interface BuildInheritedAgentEnvOptions {
  /** Env to inherit from. Defaults to the Paperclip server's `process.env`. */
  baseEnv?: NodeJS.ProcessEnv;
  /** Apply {@link ensurePathInEnv} to the merged result (most spawn sites do). */
  ensurePath?: boolean;
}
 ...
 *
 * Inherits `baseEnv` (defaults to the Paperclip server's `process.env`),
 * applies `overrides` on top, then removes every `AGENT_ENV_DENY_LIST` key.
 * The deny pass runs last on purpose: adapter/user config env must not be able
 * to reintroduce a denied secret either.
 */
export function buildInheritedAgentEnv(
  overrides: NodeJS.ProcessEnv = {},
  options: BuildInheritedAgentEnvOptions = {},
): NodeJS.ProcessEnv {
  const merged: NodeJS.ProcessEnv = { ...(options.baseEnv ?? process.env), ...overrides };
  const shaped = options.ensurePath ? { ...ensurePathInEnv(merged) } : merged;
  for (const key of Object.keys(shaped)) {
    if (AGENT_ENV_DENY_SET.has(key.toUpperCase())) delete shaped[key];
  }
  return shaped;
}

/**
 * String-only variant of {@link buildInheritedAgentEnv} for the many call sites
 * that need `Record<string, string>` (child_process `env`, SSH env maps).
 */
export function buildInheritedAgentEnvRecord(
  overrides: NodeJS.ProcessEnv = {},
  options: BuildInheritedAgentEnvOptions = {},
): Record<string, string> {
  return Object.fromEntries(
    Object.entries(buildInheritedAgentEnv(overrides, options)).filter(
...
export async function runChildProcess(
  ...
  return new Promise<RunProcessResult>((resolve, reject) => {
    // sanitizeInheritedPaperclipEnv strips the PAPERCLIP_* runtime namespace;
    // buildInheritedAgentEnv additionally drops the platform signing secrets
    // that are not PAPERCLIP_*-named (BETTER_AUTH_SECRET). See SSO-22654.
    const rawMerged: NodeJS.ProcessEnv = buildInheritedAgentEnv(opts.env, {
      baseEnv: sanitizeInheritedPaperclipEnv(process.env),
    });

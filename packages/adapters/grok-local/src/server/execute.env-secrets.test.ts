...
const {
  ...
  resolveAdapterExecutionTargetCommandForLogs: vi.fn(async () => "grok"),
...
describe("grok_local platform signing secret containment", () => {
  ...
  it("keeps platform signing secrets out of the child env while the run still completes", async () => {
    const cwd = await mkdtemp(path.join(os.tmpdir(), "paperclip-grok-env-"));
